...
/// \param Q_conv  [out] energy gain from convective exchange between layers [MJ/d]
/// \param Q_lat   [out] energy gain from latent heat exchange [MJ/d]
/// \param Q_sw_in [out] energy gain from incoming SW radiation at surface [MJ/d]
/// \param Q_lw_in [out] energy gain from incoming LW radiation at surface [MJ/d]
                                                     ...
                                                     double &Q_adv) const
{
  ...
  double A_h_new = A_new;
  double V_h_new = pRes->GetHypolimnionStorage();
  double V_h_old = pRes->GetOldHypolimnionStorage();
  if((V_h_old<=0.0) || (V_h_new<=0.0)) { //handles shallow lake, i.e. no hypolimnion layer
    isShallow = true;
    V_h_new = 0.0;
  }
  ...
  if(pHRU!=NULL) { //otherwise only simulate advective mixing + rain input
    ...
    if(isShallow) {
      V_h_new = pRes->GetLakebedThickness() * pHRU->GetArea()*M2_PER_KM2;
    } else {
      A_h_new = pRes->GetMixingArea();
      Q_dn = pRes->GetOldDownwardFlowRate();
      Q_up = pRes->GetOldUpwardFlowRate();
    }
  } else {
    V_h_new = 0.001;
  }
  ...
  Q_lat   =-(AET * DENSITY_WATER * LH_VAPOR)*A_new;                    //[MJ/d]
  Q_rain  =_aMresRain[p];                                              //[MJ/d]
  Q_adv   = (Q_up*_aMsed[p]/V_e_new - Q_dn*_aMres[p]/V_h_new);         //[MJ/d]
                                               ...
                                               const time_struct &tt) const
{
  ...
  double A_h_new(0.0), A_h_old(0.0);
  double A_h_avg = A_avg;
  double Q_dn_old(0.0), Q_up_old(0.0);
  ...
  if(pHRU!=NULL) { //otherwise only simulate advective mixing + rain input
    ...
    //Deep lake with epilimnion + hypolimnion layers (sediment ignored)
      ...
      Ts_old = ConvertVolumetricEnthalpyToTemperature(_aMsed[p]/V_h_old);
      A_h_new = pRes->GetMixingArea();
      A_h_old = pRes->GetOldMixingArea();
      A_h_avg = 0.5 * (A_h_new + A_h_old);
      zc = ((V_e_old/A_old)+(V_h_old/A_h_old))/2.0;
      zm = pRes->GetMixingDepth();
      Q_dn_old = pRes->GetOldDownwardFlowRate();
      Q_up_old = pRes->GetOldUpwardFlowRate();
  ...
  B[0]+=(1.0-0.5*tstep/V_e_old*Q_old)*_aMres[p];                     // outflow [MJ]
  ...
  double dE,dEs;
  double E_guess  =_aMres[p];
  double Es_guess =_aMsed[p];
  ...
  while (change > tolerance)
  {
    ...
    if (iter>10){
        ...
        Ts_guess= ConvertVolumetricEnthalpyToTemperature(_aMsed[p]/V_h_new);
        dE  = aMout_new[nSegments-1]*tstep;             // inflow [MJ]
        dE -= Q_new/V_e_new*_aMres[p]*tstep;            // outflow [MJ]
        dE -= Q_dn_new/V_e_new*_aMres[p]*tstep;         // downward advection [MJ]
        dE += Q_up_new/V_h_new*_aMsed[p]*tstep;         // upward advection [MJ]
        dE += _aMresRain[p]*tstep;                      // rainfall inputs [MJ]
        LW  = -STEFAN_BOLTZ*EMISS_WATER*pow(T_guess+ZERO_CELSIUS,4);
    ...
    for (p = 0; p < _pModel->GetNumSubBasins(); p++)
    {
      if ((_pModel->GetSubBasin(p)->IsGauged()) && (_pModel->GetSubBasin(p)->IsEnabled()))
      {
        ...
        if ((pRes != NULL) && (pRes->GetHRUIndex() != DOESNT_EXIST))
        {
          ...
          if (V_e_new!=0){_LAKEOUT << pctFroz          << ",";}else{_LAKEOUT<<",";}
        }
      }
    }
    _LAKEOUT << endl;
