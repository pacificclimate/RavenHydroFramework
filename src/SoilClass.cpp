                                        ...
                                        const int nConstit)
{
  ...
  ExitGracefullyIf((_Soil.sand_con<0) || (_Soil.sand_con>1.0),"AutoCalculateSoilProps: SAND_CON must be between 0 and 1",BAD_DATA_WARN);
  ExitGracefullyIf((_Soil.clay_con<0) || (_Soil.clay_con>1.0),"AutoCalculateSoilProps: CLAY_CON must be between 0 and 1",BAD_DATA_WARN);
  ExitGracefullyIf((_Soil.org_con <0) || (_Soil.org_con >1.0), "AutoCalculateSoilProps: ORG_CON must be between 0 and 1",BAD_DATA_WARN);
  ...
  bad=SetSpecifiedValue(_Soil.VIC_baseflow_ds,Stmp.VIC_baseflow_ds,Sdefault.VIC_baseflow_ds,needed,"VIC_BASEFLOW_DS");
  bad=SetSpecifiedValue(_Soil.VIC_baseflow_ws,Stmp.VIC_baseflow_ws,Sdefault.VIC_baseflow_ws,needed,"VIC_BASEFLOW_WS");
...
//
void CSoilClass::InitializeSoilProperties(soil_struct &S, bool is_template,int nConstituents)//static
{
  S.org_con =0.0;
  S.clay_con=0.0;
  S.sand_con=1.0;
  ...
  S.VIC_baseflow_ds   =DefaultParameterValue(is_template,false);//
  S.VIC_baseflow_ws   =DefaultParameterValue(is_template,false);//
                                  ...
                                  const double value)
{
  ...
  else if (!name.compare("VIC_BASEFLOW_DS"     )){S.VIC_baseflow_ds=value;}
  else if (!name.compare("VIC_BASEFLOW_WS"     )){S.VIC_baseflow_ws=value;}
...
double CSoilClass::GetSoilProperty(const soil_struct &S, string param_name,  const bool strict)
{
  ...
  else if (!name.compare("VIC_BASEFLOW_DS"     )){return S.VIC_baseflow_ds;}
  else if (!name.compare("VIC_BASEFLOW_WS"     )){return S.VIC_baseflow_ws;}
